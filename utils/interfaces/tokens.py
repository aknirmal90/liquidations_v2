...
class AaveToken(BaseContractInterface):
    ...
    def get_previous_index(self, users: List[str]) -> dict:
        """
        Returns a dict mapping user address to previous index for each provided user.

        Args:
            users: List of user addresses.

        Returns:
            Dictionary {user_address: previous_index}
        """
        call_targets = [
            {
                "method_signature": "getPreviousIndex(address)",
                "param_types": ["address"],
                "params": [user],
            }
            for user in users
