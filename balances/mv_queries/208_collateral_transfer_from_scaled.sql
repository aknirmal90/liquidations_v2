-- MV Query: Materialized View for BalanceTransfer _from (Collateral) with correct ray math scaling
...
-- where RAY = 1e27
CREATE MATERIALIZED VIEW IF NOT EXISTS aave_ethereum.mv_balances_transfer_from_scaled TO aave_ethereum.LatestBalances_v2
AS SELECT
    bt._from as user,
    bt.asset as asset,
    -- Convert to scaled (negative): -floor(value * RAY / block_index)
    sumState(toInt256(
        -1 * floor((toDecimal256(bt.value, 0) * toDecimal256('1000000000000000000000000000', 0)) / toDecimal256(bli.max_collateral_liquidityIndex, 0))
