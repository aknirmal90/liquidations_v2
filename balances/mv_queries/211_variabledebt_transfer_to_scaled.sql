-- MV Query: Materialized View for BalanceTransfer _to (VariableDebt) with correct ray math scaling
...
-- where RAY = 1e27
CREATE MATERIALIZED VIEW IF NOT EXISTS aave_ethereum.mv_variabledebt_transfer_to_scaled TO aave_ethereum.LatestBalances_v2
AS SELECT
    bt._to as user,
    bt.asset as asset,
    ...
    sumState(toInt256(
        floor((toDecimal256(bt.value, 0) * toDecimal256('1000000000000000000000000000', 0)) / toDecimal256(bli.max_variable_debt_liquidityIndex, 0))
