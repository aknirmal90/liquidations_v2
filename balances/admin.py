from django.contrib import admin
from django.utils.html import format_html

from balances.models import BalanceEvent
from utils.admin import (
    EnableDisableAdminMixin,
    format_pretty_json,
    get_explorer_address_url,
)


@admin.register(BalanceEvent)
class BalanceEventAdmin(EnableDisableAdminMixin, admin.ModelAdmin):
    list_display = (
        "name",
        "type",
        "is_enabled",
        "last_synced_block",
        "blocks_to_sync",
        "logs_count",
        "get_asset_display",
        "get_contracts_display",
        "updated_at",
    )

    list_filter = ("is_enabled", "updated_at", "type")

    search_fields = (
        "asset",
        "name",
        "signature",
        "topic_0",
    )

    fieldsets = (
        ("Basic Information", {"fields": ("name", "is_enabled", "asset", "type")}),
        (
            "Sync Status",
            {"fields": ("last_synced_block", "blocks_to_sync", "logs_count")},
        ),
        (
            "Event Details",
            {"fields": ("signature", "topic_0", "abi_display", "contract_addresses")},
        ),
        ("Timestamps", {"fields": ("created_at", "updated_at")}),
    )

    readonly_fields = (
        "created_at",
        "updated_at",
        "blocks_to_sync",
        "name",
        "signature",
        "abi",
        "topic_0",
        "abi_display",
        "contract_addresses",
        "logs_count",
        "is_enabled",
        "get_contracts_display",
        "get_asset_display",
    )

    def abi_display(self, obj):
        return format_pretty_json(obj.abi)

    abi_display.short_description = "ABI"

    def get_contracts_display(self, obj):
        contract_links = []
        for contract in obj.contract_addresses:
            contract_links.append(get_explorer_address_url(contract))
        return format_html("<br>".join(contract_links))

    get_contracts_display.short_description = "Contract Addresses"

    def get_asset_display(self, obj):
        if obj.asset:
            return get_explorer_address_url(obj.asset)

    get_asset_display.short_description = "Asset"
