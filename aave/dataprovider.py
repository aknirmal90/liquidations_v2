...
class AaveDataProvider:
    ...
    def _make_payload(self, contract_name: str, function_name: str, params: list, id: int = 0):
        contract_address = getattr(self, f"{contract_name}_contract_address")
    ...
    def get_batch_response(self, contract_name: str, function_name: str, params_list: list):
        function_abi = web3.utils.filter_abi_by_name(function_name, self.abi)[0]
        payloads = [
            self._make_payload(contract_name, function_name, params, i) for i, params in enumerate(params_list)
        ]
