...
#         self.data_provider_contract_address = Configuration.get(
#             f"AAVE_DATA_PROVIDER_{self.network.chain_id}"
#         )
#         self.price_oracle_contract_address = Configuration.get(
#             f"AAVE_PRICE_ORACLE_{self.network.chain_id}"
#         )
#         self.pool_contract_address = Configuration.get(
#             f"AAVE_POOL_CONTRACT_{self.network.chain_id}"
#         )
...
#     ):
#         contract_address = contract_address or getattr(
#             self, f"{contract_name}_contract_address"
#         )
...
#                     "data": add_0x_prefix(
#                         web3.Web3.keccak(text=function_signature).hex()[:8]
#                         + encoded_params
#                     ),
#                 },
#                 "latest",
...
#         payloads = [
#             self._make_payload(
#                 contract_name, function_name, params, i, contract_address
#             )
#             for i, params in enumerate(params_list)
#         ]
#         responses = requests.post(self.network.rpc, json=payloads)
#         return [
#             {
#                 "id": response.get("id"),
#                 "result": decode_any(
#                     get_decoded_params(function_abi, response.get("result"))
#                 ),
#             }
...
#             params_list=[[reserve, user] for user in users],
#         )
...
#             params_list=[[reserve] for reserve in reserves],
#         )
...
#             params_list=[[asset] for asset in assets],
#         )
...
#             params_list=[[user] for user in users],
#         )
