...
class BalanceUtils:
    def _update_existing_records(
        ...
        instances_to_update = model_class.objects.filter(
            network=event.network, address__in=users, asset_id=asset_id
        )
        ...
        for instance in instances_to_update:
            ...
            if collateral_index:
                ...
                instance.collateral_amount_live = instance.get_scaled_balance(
                    "collateral"
                )
                instance.collateral_amount_live_with_liquidation_threshold = (
                    instance.get_collateral_amount_live_with_liquidation_threshold()
                )
                instance.price_in_nativeasset = (
                    instance.asset.price_in_nativeasset or Decimal("0.0")
                )
                instance.collateral_health_factor = (
                    instance.get_collateral_health_factor()
                )
        ...
        if collateral_update_instances:
            ...
            logger.info(
                f"Updated collateral for {len(collateral_update_instances)} records for asset {asset_id}"
            )
        ...
        if borrow_update_instances:
            ...
            logger.info(
                f"Updated borrow for {len(borrow_update_instances)} records for asset {asset_id}"
            )

        return list(set(collateral_update_instances + borrow_update_instances))

    def _create_new_records(
        model_class, event, asset_id, new_addresses, balances, liquidity_indices
    ):
        ...
        for address in new_addresses:
            ...
            liquidity_data = (
                liquidity_indices[asset_id][address] if liquidity_indices else {}
            )
            collateral_index = liquidity_data.get("collateral")
            borrow_index = liquidity_data.get("borrow")

            instance = model_class(
                network=event.network, address=address, asset_id=asset_id
            )
...
class aaveAdapter(BalanceUtils):
    ...
    @classmethod
    def _bulk_create_and_update_metadata(
        cls, model_class, defaults_list, update_fields
    ):
    ...
    def parse_CollateralConfigurationChanged(cls, event: Event, logs: List[Dict]):
        ...
        cls._bulk_create_and_update_metadata(
            model_class, defaults_list, ["liquidation_threshold", "liquidation_bonus"]
        )
    ...
    def parse_AssetSourceUpdated(cls, event: Event, logs: List[Dict]):
        ...
        for defaults in defaults_list:
            try:
                ...
                logger.error(
                    f"Failed to process record for {defaults['asset']}: {str(e)}"
                )
    ...
    def parse_EModeAssetCategoryChanged(cls, event: Event, logs: List[Dict]):
        ...
        cls._bulk_create_and_update_metadata(
            model_class, defaults_list, ["emode_category"]
        )
    ...
    def parse_EModeCategoryAdded(cls, event: Event, logs: List[Dict]):
        ...
        for log in logs:
            ...
            model_class.objects.filter(
                network=event.network, emode_category=log.args.categoryId
            ).update(
    ...
    def parse_PriceCapUpdated(cls, event: Event, logs: List[Dict]):
        ...
        for log in logs:
            model_class.objects.filter(
                network=event.network, pricesource=log.address
            ).update(
    ...
    def parse_LiquidationCall(cls, event: Event, logs: List[Dict]):
        ...
        approximate_block_timestamp = ApproximateBlockTimestamp.objects.get(
            network=event.network
        )
        ...
        for log in logs:
            collateral_asset = Asset.get_by_address(
                network_name=network_name, token_address=log.args.collateralAsset
            )
            debt_asset = Asset.get_by_address(
                network_name=network_name, token_address=log.args.debtAsset
            )

            block_timestamp = approximate_block_timestamp.get_timestamps(
                [log.blockNumber]
            )[log.blockNumber]
            ...
            try:
                liquidated_collateral_amount_in_usd = (
                    ...
                    * collateral_asset.price_in_nativeasset
                    / collateral_asset.decimals
                )
                debt_to_cover_in_usd = (
                    debt_to_cover
                    * debt_asset.price_in_nativeasset
                    / debt_asset.decimals
                )
                collateral_returned = (
                    liquidated_collateral_amount
                ...
                profit_in_usd = (
                    (liquidated_collateral_amount - collateral_returned)
                    * collateral_asset.price_in_nativeasset
                    / collateral_asset.decimals
                )
            except Exception:
    ...
    def _update_collateral_indices(
        ...
        user = (
            log.args.onBehalfOf
            if hasattr(log.args, "onBehalfOf")
            else getattr(log.args, "from")
        )
        ...
        if liquidity_index > liquidity_indices[asset_id][user].get(
            "collateral", Decimal("0.0")
        ):
    ...
    def _update_asset_liquidity_indices(cls, max_indices):
        ...
        for asset in existing_assets:
            if (
                ...
                and max_indices["collateral"][asset.id]
                > asset.collateral_liquidity_index
            ):
        ...
        if collateral_updates:
            Asset.objects.bulk_update(
                collateral_updates, fields=["collateral_liquidity_index"]
            )

        if borrow_updates:
            Asset.objects.bulk_update(borrow_updates, fields=["borrow_liquidity_index"])
    ...
    def parse_BalanceTransfer(cls, event: Event, logs: List[Dict]):
        ...
        balances = defaultdict(
            lambda: defaultdict(lambda: defaultdict(lambda: Decimal("0.0")))
        )
        ...
        for log in logs:
            ...
            liquidity_indices[asset_id][to_addr] = {
                "collateral": collateral_liquidity_index
            }
            liquidity_indices[asset_id][from_addr] = {
                "collateral": collateral_liquidity_index
            }

            balances[asset_id][to_addr]["collateral"] += (
                Decimal(log.args.value) / asset.decimals
            )
            balances[asset_id][from_addr]["collateral"] -= (
                Decimal(log.args.value) / asset.decimals
            )
    ...
    def parse_ReserveUsedAsCollateralDisabled(cls, event: Event, logs: List[Dict]):
        ...
        for log in logs:
            ...
            asset = Asset.get_by_address(
                network_name=event.network.name, token_address=asset_id
            )
            user = log.args.user

            if user not in latest_collateral_disabled[asset_id] or is_latest_log(
                log, latest_collateral_disabled[asset_id][user]
            ):
        ...
        for asset_id in latest_collateral_disabled:
            ...
            user_reserves = model_class.objects.filter(
                network=event.network, address__in=users, asset_id=asset.id
            )
    ...
    def parse_ReserveUsedAsCollateralEnabled(cls, event: Event, logs: List[Dict]):
        ...
        for log in logs:
            ...
            if user not in latest_collateral_enabled[asset_id] or is_latest_log(
                log, latest_collateral_enabled[asset_id][user]
            ):
            ...
            for user_reserve in user_reserves:
                ...
                if (
                    ...
                    user_reserve.collateral_amount_live = (
                        user_reserve.get_scaled_balance("collateral")
                    )

                    user_reserve.collateral_amount_live_with_liquidation_threshold = user_reserve.get_collateral_amount_live_with_liquidation_threshold()
                    user_reserve.price_in_nativeasset = price_in_nativeasset
                    user_reserve.collateral_health_factor = (
                        user_reserve.get_collateral_health_factor()
                    )
                    instances_to_update.append(user_reserve)
    ...
    def parse_UserEModeSet(cls, event: Event, logs: List[Dict]):
        ...
        for log in logs:
            ...
            if user not in latest_emode_categories:
                ...
                current_log=log, latest_log=latest_emode_categories[user]
            ):
                latest_emode_categories[user] = log
        ...
        existing_users = set(
            model_class.objects.filter(
                network=event.network, address__in=list(latest_emode_categories.keys())
        ...
        instances_to_update = []
        user_reserves = model_class.objects.filter(
            network=event.network, address__in=existing_users
        ).select_related("asset")
        ...
        for user_reserve in user_reserves:
            ...
            if (
                ...
                user_reserve.collateral_amount_live = user_reserve.get_scaled_balance(
                    "collateral"
                )
                ...
                user_reserve.price_in_nativeasset = (
                    user_reserve.asset.price_in_nativeasset or Decimal("0.0")
                )
                user_reserve.collateral_health_factor = (
                    user_reserve.get_collateral_health_factor()
                )
